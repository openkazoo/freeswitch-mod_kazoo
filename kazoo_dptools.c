...
static void base_set(switch_core_session_t *session, const char *data, int urldecode, switch_stack_t stack)
{
	...
	if (zstr(data)) {
		...
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "%s set [%s]=[%s] => [%s]\n",
						  switch_channel_get_name(channel), var, val, expanded ? expanded : "UNDEF");
	...
	if (zstr(data)) {
		...
		if (val) {
			...
			if (!kz_is_exported(session, var)) {
				...
				switch_channel_export_variable_var_check(channel, var, expanded, SWITCH_EXPORT_VARS_VARIABLE,
														 SWITCH_FALSE);
			} else {
...
void kz_multiset(switch_core_session_t *session, const char *data, int urldecode)
{
	...
	if (delim != '\0') {
		...
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "kz_multiset() with empty args\n");
	}
}
...
void kz_uuid_multiset(switch_core_session_t *session, const char *data, int urldecode)
{
	...
	if (arg == NULL) {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
						  "kz_uuid_multiset() with invalid args\n");
		return;
	}
	*arg = '\0';
	arg++;
	...
	if (zstr(arg0)) {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
						  "kz_uuid_multiset() with invalid uuid\n");
		return;
	}
	...
	if (delim != '\0') {
		...
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
						  "kz_uuid_multiset() with empty args\n");
	}
}
...
SWITCH_STANDARD_APP(multiunset_function)
{
	...
	if (delim != '\0') {
		...
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
						  "multiunset_function() with empty args\n");
	}
}
...
void kz_export(switch_core_session_t *session, const char *data, int urldecode)
{
	...
	if (delim != '\0') {
		...
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "kz_export() with empty args\n");
	}
}
...
SWITCH_STANDARD_APP(kz_moh_function)
{
	...
	case SWITCH_STATUS_SUCCESS:
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG1,
						  "MOH switch_ivr_play_file(): success\n");
	...
	case SWITCH_STATUS_BREAK:
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG1,
						  "MOH switch_ivr_play_file(): break\n");
		switch_channel_set_variable(channel, SWITCH_CURRENT_APPLICATION_RESPONSE_VARIABLE, "MOH FILE PLAYED");
	...
	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG1, "MOH pos: %" SWITCH_INT64_T_FMT "\n",
					  fh.pos);
	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG1,
					  "MOH sample_count %" SWITCH_SIZE_T_FMT "\n", fh.sample_count);
...
SWITCH_STANDARD_APP(kz_audio_bridge_function)
{
	...
	status = switch_ivr_originate(session, &peer_session, &cause, data, 0, NULL, NULL, NULL, NULL, NULL, SOF_NONE, NULL,
								  NULL);

	if (status != SWITCH_STATUS_SUCCESS) {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "Originate failed: %s\n",
						  switch_channel_cause2str(cause));

		switch_channel_set_variable(caller_channel, "originate_failed_cause", switch_channel_cause2str(cause));
		switch_channel_set_variable(caller_channel, SWITCH_CURRENT_APPLICATION_RESPONSE_VARIABLE,
									switch_channel_cause2str(cause));
		...
		while (1) {
			...
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "A State %s: %s =>  %s , %s\n",
							  switch_channel_state_name(a_running_state), switch_channel_state_name(a_state), uuid,
							  peer_uuid);
			...
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "B State %s: %s =>  %s , %s\n",
							  switch_channel_state_name(b_running_state), switch_channel_state_name(b_state), uuid,
							  peer_uuid);
			...
			if (b_state >= CS_HANGUP) {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "B Hangup: %s , %s\n", uuid,
								  peer_uuid);
				switch_channel_set_variable(caller_channel, SWITCH_CURRENT_APPLICATION_RESPONSE_VARIABLE,
											switch_channel_cause2str(switch_channel_get_cause(peer_channel)));
				break;
			}
...
static switch_status_t kz_att_xfer_on_dtmf(switch_core_session_t *session, void *input, switch_input_type_t itype,
		...
		if (dtmf->digit == *keys->attxfer_conf_key) {
			...
			if ((extension = switch_caller_extension_new(peer_session, app, app_arg)) == 0) {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_CRIT,
								  "Failed to allocate memory from session pool\n");
				abort();
			}
...
{
	switch_channel_set_variable(channel, SWITCH_ATT_XFER_RESULT_VARIABLE,
								status == SWITCH_STATUS_SUCCESS ? "success" : "failure");
}
...
void *SWITCH_THREAD_FUNC kz_att_thread_run(switch_thread_t *thread, void *obj)
{
	...
	if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, "sofia::transferee") == SWITCH_STATUS_SUCCESS) {
		...
		switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "att_xfer_replaced_call_id",
	...
	if (!switch_channel_ready(channel)) {
		...
		// switch_channel_set_variable_printf(b_channel, "xfer_uuids", "%s %s",
		// switch_core_session_get_uuid(peer_session), switch_core_session_get_uuid(session));
		switch_channel_set_variable_printf(channel, "xfer_uuids", "%s %s", switch_core_session_get_uuid(peer_session),
										   bond);
...
void add_kz_dptools(switch_loadable_module_interface_t **module_interface)
{
	...
	SWITCH_ADD_APP(app_interface, "kz_set_encoded", SET_SHORT_DESC, SET_LONG_DESC, set_encoded_function, SET_SYNTAX,
				   SAF_SUPPORT_NOMEDIA | SAF_ROUTING_EXEC | SAF_ZOMBIE_EXEC);
	SWITCH_ADD_APP(app_interface, "kz_multiset", MULTISET_SHORT_DESC, MULTISET_LONG_DESC, multiset_function,
				   MULTISET_SYNTAX, SAF_SUPPORT_NOMEDIA | SAF_ROUTING_EXEC | SAF_ZOMBIE_EXEC);
	SWITCH_ADD_APP(app_interface, "kz_multiset_encoded", MULTISET_SHORT_DESC, MULTISET_LONG_DESC,
				   multiset_encoded_function, MULTISET_SYNTAX,
				   SAF_SUPPORT_NOMEDIA | SAF_ROUTING_EXEC | SAF_ZOMBIE_EXEC);
	...
	SWITCH_ADD_APP(app_interface, "kz_prefix_unset", PREFIX_UNSET_SHORT_DESC, PREFIX_UNSET_LONG_DESC,
				   prefix_unset_function, PREFIX_UNSET_SYNTAX,
				   SAF_SUPPORT_NOMEDIA | SAF_ROUTING_EXEC | SAF_ZOMBIE_EXEC);
	SWITCH_ADD_APP(app_interface, "kz_uuid_multiset", UUID_MULTISET_SHORT_DESC, UUID_MULTISET_LONG_DESC,
				   uuid_multiset_function, UUID_MULTISET_SYNTAX,
				   SAF_SUPPORT_NOMEDIA | SAF_ROUTING_EXEC | SAF_ZOMBIE_EXEC);
	SWITCH_ADD_APP(app_interface, "kz_uuid_multiset_encoded", UUID_MULTISET_SHORT_DESC, UUID_MULTISET_LONG_DESC,
				   uuid_multiset_encoded_function, UUID_MULTISET_SYNTAX,
				   SAF_SUPPORT_NOMEDIA | SAF_ROUTING_EXEC | SAF_ZOMBIE_EXEC);
	SWITCH_ADD_APP(app_interface, "kz_endless_playback", KZ_ENDLESS_PLAYBACK_SHORT_DESC, KZ_ENDLESS_PLAYBACK_LONG_DESC,
				   kz_endless_playback_function, KZ_ENDLESS_PLAYBACK_SYNTAX, SAF_NONE);
	SWITCH_ADD_APP(app_interface, "kz_restore_caller_id", NOOP_SHORT_DESC, NOOP_LONG_DESC,
				   kz_restore_caller_id_function, NOOP_SYNTAX, SAF_NONE);
	SWITCH_ADD_APP(app_interface, "noop", NOOP_SHORT_DESC, NOOP_LONG_DESC, noop_function, NOOP_SYNTAX, SAF_NONE);
	SWITCH_ADD_APP(app_interface, "kz_bridge", "Bridge Audio", "Bridge the audio between two sessions",
				   kz_audio_bridge_function, "<channel_url>", SAF_SUPPORT_NOMEDIA | SAF_SUPPORT_TEXT_ONLY);
	SWITCH_ADD_APP(app_interface, "kz_bridge_uuid", "Bridge Audio", "Bridge the audio between two sessions",
				   kz_audio_bridge_uuid_function, "<channel_url>", SAF_SUPPORT_NOMEDIA | SAF_SUPPORT_TEXT_ONLY);
	SWITCH_ADD_APP(app_interface, "kz_att_xfer", "Attended Transfer", "Attended Transfer", kz_att_xfer_function,
